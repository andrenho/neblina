...
#define JSON_FIELD_OFFSET_ITEM_SIZE 10
...
struct json_field_offset_item json_field_offset_item[JSON_FIELD_OFFSET_ITEM_SIZE] = {
    {0, sizeof(struct Service), 6, "", "", "Service", 0},
    {offsetof(struct Service, name), sizeof(sstr_t), 4, "sstr_t", "name", "Service", 0},
    {offsetof(struct Service, active), sizeof(int), 7, "int", "active", "Service", 0},
    {offsetof(struct Service, port), sizeof(int), 0, "int", "port", "Service", 0},
    {offsetof(struct Service, openToWorld), sizeof(int), 7, "int", "openToWorld", "Service", 0},
    {offsetof(struct Service, loggingColor), sizeof(int), 0, "int", "loggingColor", "Service", 0},
    {0, sizeof(struct Config), 6, "", "", "Config", 0},
    {offsetof(struct Config, services), sizeof(struct Service), 6, "Service", "services", "Config", 1},
    {offsetof(struct Config, services_len), sizeof(int), 0, "int", "services_len", "Config", 0},
    {0, 0, 0, NULL, NULL, NULL, 0}};
int json_entry_hash_size = 15;
int json_entry_hash[15] = {5, -1, 0, 1, -1, -1, 6, 4, 7, 8, -1, -1, -1, 3, 2};
...
int json_marshal_indent_Service(struct Service* obj, int indent, int curindent, sstr_t out) {
    ...
    sstr_append_of(out, "\"", 1);
    sstr_json_escape_string_append(out, obj->name);
    sstr_append_of(out, "\"", 1);
    sstr_append_cstr(out, ",");
    sstr_append_of_if(out, "\n", 1, indent);
    sstr_append_indent(out, curindent);
    sstr_append_cstr(out, "\"active\":");
    if (obj->active) {
    ...
    sstr_append_of_if(out, "\n", 1, indent);
    sstr_append_indent(out, curindent);
    sstr_append_cstr(out, "\"port\":");
    sstr_append_int_str(out, obj->port);
    sstr_append_cstr(out, ",");
    sstr_append_of_if(out, "\n", 1, indent);
    sstr_append_indent(out, curindent);
    sstr_append_cstr(out, "\"openToWorld\":");
    if (obj->openToWorld) {
        sstr_append_cstr(out, "true");
    } else {
        sstr_append_cstr(out, "false");
    ...
    sstr_append_of_if(out, "\n", 1, indent);
    sstr_append_indent(out, curindent);
    sstr_append_cstr(out, "\"loggingColor\":");
    sstr_append_int_str(out, obj->loggingColor);
    sstr_append_of_if(out, "\n", 1, indent);
    curindent -= indent;
    sstr_append_indent(out, curindent);
    sstr_append_of(out, "}", 1);
...
int json_unmarshal_array_Service(sstr_t in, struct Service** obj, int *len) {
    *len = 0;
    ...
    ar_param.struct_name = "Service";
    ar_param.field_name="";
    int r = json_unmarshal_array_internal(in, &pos, &ar_param, len, txt);
    if (r < 0) {
#ifdef JSON_DEBUG
        printf("ERROR: %s", sstr_cstr(txt));
#endif
...
int json_marshal_array_indent_Service(struct Service* obj, int len, int indent, int curindent, sstr_t out) {
    ...
    curindent += indent;
    for (i = 0; i < len; i++) {
        json_marshal_indent_Service(&obj[i], indent, curindent, out);
        if (i < len - 1) {
            sstr_append_cstr(out, ",");
        }
        sstr_append_of_if(out, "\n", 1, indent);
...
int Config_clear(struct Config*obj) {
    int i;
    for (i = 0; i < obj->services_len; i++) {
       Service_clear(&obj->services[i]);
    }
    free(obj->services);
    obj->services = NULL;
    obj->services_len = 0;
...
int json_marshal_indent_Config(struct Config* obj, int indent, int curindent, sstr_t out) {
    ...
    curindent += indent;
    sstr_append_indent(out, curindent);
    sstr_append_cstr(out, "\"services\":");
    json_marshal_array_indent_Service(obj->services, obj->services_len, indent, curindent, out);
    sstr_append_of_if(out, "\n", 1, indent);
    curindent -= indent;
    sstr_append_indent(out, curindent);
    sstr_append_of(out, "}", 1);
...
int json_marshal_array_indent_Config(struct Config* obj, int len, int indent, int curindent, sstr_t out) {
    ...
    curindent += indent;
    for (i = 0; i < len; i++) {
        json_marshal_indent_Config(&obj[i], indent, curindent, out);
        if (i < len - 1) {
            sstr_append_cstr(out, ",");
        }
        sstr_append_of_if(out, "\n", 1, indent);
