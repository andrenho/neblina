...
/* assemble command in printf(3) style */
{
	if (errlevel <= debuglevel) {
		...
		buf[BUFSIZ - 1] = '\0';
		va_start(ap, fmt);
		(void)vsnprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
		                fmt, ap);
		va_end(ap);
...
static int json_internal_read_object(const char *cp,
...
#ifdef DEBUG_ENABLE
	char *statenames[] = {
	    "init",         "await_attr",    "in_attr",
	    "await_value",  "in_val_string", "in_escape",
	    "in_val_token", "post_val",      "post_element",
	};
#endif /* DEBUG_ENABLE */
		...
		if (!cursor->nodefault) {
			...
			if (lptr != NULL) {
				...
				case t_uinteger:
					...
					break;
				case t_short:
					memcpy(lptr, &cursor->dflt.shortint,
					       sizeof(short));
					break;
				case t_ushort:
				...
				case t_string:
					...
					break;
				case t_boolean:
					memcpy(lptr, &cursor->dflt.boolean,
					       sizeof(bool));
					break;
				case t_character:
	...
	for (; *cp != '\0'; cp++) {
		...
		case in_attr:
			if (pattr == NULL) {
				/* don't update end here, leave at attribute
				 * start */
			...
			if (*cp == '"') {
				*pattr++ = '\0';
				json_debug_trace(
				    (1, "Collected attribute name %s\n",
				     attrbuf));
				for (cursor = attrs; cursor->attribute != NULL;
				     cursor++) {
					json_debug_trace(
					    (2, "Checking against %s\n",
					     cursor->attribute));
					if (strcmp(cursor->attribute,
					           attrbuf) == 0) {
						break;
					}
					if (strcmp(cursor->attribute, "") ==
				...
				if (cursor->attribute == NULL) {
					...
					/* don't update end here, leave at
					 * attribute start */
		...
		case await_value:
			if (isspace((unsigned char)*cp) || *cp == ':') {
				...
				if (cursor->type != t_array) {
					json_debug_trace(
					    (1, "Saw [ when not expecting "
					        "array.\n"));
					if (end != NULL)
						*end = cp;
					return JSON_ERR_NOARRAY;
				}
				substatus = json_read_array(
				    cp, &cursor->addr.array, &cp);
				if (substatus != 0) {
					return substatus;
				}
				state = post_element;
			} else if (cursor->type == t_array) {
				json_debug_trace((1, "Array element was "
				                     "specified, but no [.\n"));
				if (end != NULL) {
					*end = cp;
				}
				...
				substatus = json_read_object(
				    cp, cursor->addr.attrs, &cp);
				if (substatus != 0) {
					return substatus;
				}
				--cp; // last } will be re-consumed by cp++ at
				      // end of loop
				state = post_element;
			} else if (cursor->type == t_object) {
				json_debug_trace((1, "Object element was "
				                     "specified, but no {.\n"));
				if (end != NULL) {
					*end = cp;
				}
		...
		case in_escape:
			...
			case 'u':
				cp++; /* skip the 'u' */
				for (n = 0; n < 4 && isxdigit((unsigned char)*cp); n++) {
					uescape[n] = *cp++;
				}
		...
		case in_val_token:
			...
			break;
		case post_val:
			// Ignore whitespace after either string or token
			// values.
		  if (isspace((unsigned char)*cp)) {
				while (*cp != '\0' &&
			 ...
			 * correct spec; our dialect allows there to be any
			 * number of adjacent ones with the same attrname but
			 * different types.  Here's where we try to seek forward
			 * for a matching type/attr pair if we're not looking at
			 * one.
			 */
			for (;;) {
				...
				if (cursor[1].attribute ==
				    NULL) /* out of possiblities */
					break;
			...
			if (value_quoted &&
				...
				json_debug_trace(
				    (1, "Saw quoted value when expecting"
				        " non-string.\n"));
			...
			if (!value_quoted &&
				...
				json_debug_trace(
				    (1, "Didn't see quoted value when expecting"
				        " string.\n"));
			...
			if (cursor->map != 0) {
				...
				json_debug_trace((
				    1,
				    "Invalid enumerated value string \"%s\".\n",
				    valbuf));
			...
			if (cursor->type == t_check) {
				...
				lptr =
				    json_target_address(cursor, parent, offset);
			}
			if (lptr != NULL) {
				switch (cursor->type) {
				case t_integer: {
				...
				case t_boolean: {
					bool tmp =
					    (strcmp(valbuf, "true") == 0 ||
					     strtol(valbuf, NULL, 0));
					memcpy(lptr, &tmp, sizeof(bool));
				} break;
				case t_character:
					if (strlen(valbuf) > 1) {
						/* don't update end here, leave
						 * at value start */
						return JSON_ERR_STRLONG;
					} else {
						lptr[0] = valbuf[0];
...
int json_read_array(const char *cp, const struct json_array_t *arr,
	...
	for (offset = 0; offset < arr->maxlen; offset++) {
		...
		case t_string:
			...
			for (; tp - arr->arr.strings.store <
			       arr->arr.strings.storelen;
			     tp++) {
				if (*cp == '"') {
					++cp;
					*tp++ = '\0';
			...
			json_debug_trace(
			    (1, "Bad string syntax in string list.\n"));
			return JSON_ERR_BADSTRING;
		stringend:
			break;
		case t_object:
		case t_structobject:
			substatus = json_internal_read_object(
			    cp, arr->arr.objects.subtype, arr, offset, &cp);
			if (substatus != 0) {
				if (end != NULL) {
					end = &cp;
				}
		...
		case t_uinteger:
			...
			break;
		case t_short:
			arr->arr.shorts.store[offset] =
			    (short)strtol(cp, &ep, 0);
			if (ep == cp) {
				return JSON_ERR_BADNUM;
			} else {
				cp = ep;
		...
		case t_time:
			...
			arr->arr.reals.store[offset] =
			    iso8601_to_unix((char *)cp);
		...
		if (*cp == ']') {
			...
			json_debug_trace(
			    (1, "Bad trailing syntax on array.\n"));
...
breakout:
	...
	json_debug_trace(
	    (1, "leaving json_read_array() with %d elements\n", arrcount));
	return 0;
}
